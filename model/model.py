...
class RNNPredictor(nn.Module):
    ...
    def __init__(self, rnn_type, enc_inp_size, rnn_inp_size, rnn_hid_size, dec_out_size, nlayers, dropout=0.5,
        ...
        if rnn_type in ['LSTM', 'GRU']:
            ...
            self.rnn = SRU(input_size=rnn_inp_size,hidden_size=rnn_hid_size,num_layers=nlayers,dropout=dropout, layer_norm=True)
        else:
            try:
    ...
    def save_checkpoint(self,state, is_best):
        ...
        args = state['args']
        checkpoint_dir = Path(args.save_path,args.model,args.data,'checkpoint')
        checkpoint_dir.mkdir(parents=True,exist_ok=True)
        checkpoint = checkpoint_dir.joinpath(args.filename).with_suffix('.pth')

        torch.save(state, checkpoint)
        if is_best:
            model_best_dir = Path(args.save_path,args.model,args.data,'model_best')
